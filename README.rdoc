...
Forum Monster is a simple forum generator written in Rails 4. The goal of Forum Monster, is to provide a simple, and easy to setup forum application without having to dictate how your site it setup.
...
====What's new in Forum Monster for Rails 4?
- Category becomes ForumCategory to avoid possibile conflicts with an existing app.
- Forum monster supports translation now! Language availables are english and french for now. Be free to add translations into your language!
- Post aren't destroyed anymore, they are logically deleted only.
- Now, user can only edit or delete its owns posts. However, most actions are still available to other users. Please read the Authorization section to know more about it.
...
* Forum Monster does no authorization. Most actions are available to all users, even logged out users. ( Although, users who are not logged in cannot post, or create topics. )
